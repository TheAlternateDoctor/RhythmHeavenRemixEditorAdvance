    ...
    if (clickOccupation is ClickOccupation.SelectionDrag) {
        ...
        if ((!clickOccupation.isPlacementValid() || clickOccupation.isInDeleteZone()) && !overStoreArea) {
            ...
            shapeRenderer.prepareStencilMask(batch) {
                ...
                val scale = 2f
                val w = tex.width.toFloat() / RHREfresh.WIDTH * camera.viewportWidth / scale
                val h = tex.height.toFloat() / RHREfresh.HEIGHT * camera.viewportHeight / scale
                for (x in 0..(RHREfresh.WIDTH / tex.width * scale).roundToInt() + 2) {
                    for (y in 0..(RHREfresh.HEIGHT / tex.height * scale).roundToInt() + 2) {
