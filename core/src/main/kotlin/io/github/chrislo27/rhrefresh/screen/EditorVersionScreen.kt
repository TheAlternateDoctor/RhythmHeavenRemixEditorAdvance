...
class EditorVersionScreen(main: RHREfreshApplication)
    : ToolboksScreen<RHREfreshApplication, EditorVersionScreen>(main) {
    ...
    init {
        ...
        longChangelogButton = Button(palette, stage.bottomStage, stage.bottomStage).apply {
            this.leftClickAction = { _, _ ->
                Gdx.net.openURI(RHREfresh.GITHUB_RELEASES)
            ...
            override fun render(screen: EditorVersionScreen, batch: SpriteBatch, shapeRenderer: ShapeRenderer) {
                if (lastGhVer != main.githubVersion || text.isEmpty()) {
                    ...
                    val currentVer: String = (if (ghVer.isUnknown)
                        "[LIGHT_GRAY]"
                    else if (ghVer <= RHREfresh.VERSION)
                        "[CYAN]"
                    else
                        "[ORANGE]") + "${RHREfresh.VERSION}[]"
                    ...
                    val newState: State =
                            if (ghVer.isUnknown)
                                State.CHECKING
                            else if (ghVer <= RHREfresh.VERSION && !RHREfresh.triggerUpdateScreen)
                                State.GOOD
                            else
                                State.AVAILABLE
