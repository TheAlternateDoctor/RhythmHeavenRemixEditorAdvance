...
import io.github.chrislo27.rhrefresh.stage.bg.Particle
import io.github.chrislo27.rhrefresh.stage.bg.ParticleBasedBackground
...
class EventScreen(main: RHREfreshApplication)
    : ToolboksScreen<RHREfreshApplication, EventScreen>(main), HidesVersionText {
    ...
    companion object {
        ...
        fun getPossibleEvent(main: RHREfreshApplication, nextScreen: ToolboksScreen<*, *>?): EventScreen? {
            ...
            // Event check
            val today: LocalDate = when (RHREfresh.immediateEvent) {
    ...
    private val camera = OrthographicCamera().apply {
        viewportWidth = RHREfresh.WIDTH.toFloat()
        viewportHeight = RHREfresh.HEIGHT.toFloat()
    ...
    fun loadEventJson(eventType: EventType, file: FileHandle, nextScreen: ToolboksScreen<*, *>?): Boolean {
        return try {
            ...
            this.canContinue = if (eventType.canImmediatelyContinue) 0f else if (main.preferences.getInteger(PreferenceKeys.EVENT_PREFIX + eventType.name, 0) == NOW.year && (RHREfresh.immediateEvent % 2 != 0 || RHREfresh.immediateEvent == 0)) 0f else -1f
        ...
        if (canContinue > 0f) {
            ...
            when (eventType) {
                EventType.ANNIVERSARY -> {
                    ...
                    val years = NOW.year - RHREfresh.RHRE_ANNIVERSARY.year
                    val rhre = "RHRE"
    ...
    class WinterBackground(id: String, maxParticles: Int = 128,
        ...
        override fun renderBackground(camera: OrthographicCamera, batch: SpriteBatch, shapeRenderer: ShapeRenderer, delta: Float) {
            ...
            val ratioX = width / RHREfresh.WIDTH
            val ratioY = height / RHREfresh.HEIGHT
